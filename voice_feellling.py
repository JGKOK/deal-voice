...
class DialogueRecognitionSystem:
    ...
    def process_audio_file(self, audio_file_path):
        ...
        for dialogue in dialogues:
            try:
                # 情感分析
                emotion_result = self.emotion_model.generate(
                    input=audio_file_path,
                    begin_time=dialogue['start'],
                    end_time=dialogue['end'],
                    granularity="utterance"
                )
                emotion = self._parse_emotion(emotion_result) if emotion_result else "unknown"

                # 标点恢复
                punc_result = self.punc_model.generate(input=dialogue['text'])
                punc_text = punc_result[0]['text'] if punc_result else dialogue['text']
        ...
        final_dialogues.sort(key=lambda x: x['start'])

        end_time = time.time()
        print(f"\n处理完成，总耗时: {end_time - start_time:.2f}秒")
        print(f"最终生成 {len(final_dialogues)} 条带情感的对话记录")
    ...
    def _parse_emotion(self, emotion_result):
        """解析情感分析结果"""
        if not emotion_result or not isinstance(emotion_result, list):
            return "unknown"

        # emotion2vec输出格式示例: [{'emotion': 'neutral(85%),happy(15%)'}]
        emotion_str = emotion_result[0].get('emotion', '')
        if not emotion_str:
            return "unknown"

        # 提取置信度最高的情绪
        emotions = emotion_str.split(',')
        primary_emotion = emotions[0].split('(')[0] if emotions else "unknown"
        return primary_emotion.lower()
    ...
    def print_dialogue(self, dialogues):
        ...
        for dialogue in dialogues:
            print(
                f"[{dialogue['speaker']}]({dialogue['start']:.2f}-{dialogue['end']:.2f}s) [{dialogue['emotion']}]: {dialogue['text']}")
        print("=" * 80)


if __name__ == "__main__":
    print("正在初始化对话识别系统（含情感分析）...")
    drs = DialogueRecognitionSystem()

    audio_file = "/home/ysz/FunASR/tests/demo/input/2r06z-hhlkv.wav"
    ...
    # 保存结果到文件
    output_file = "dialogue_with_emotion.txt"
